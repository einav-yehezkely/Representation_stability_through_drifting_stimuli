...
def classify_images(model, csv_path, clusters=False):
    ...
    for i, row in df.iterrows():
        ...
        with torch.no_grad():
            # Forward pass: run the image through the model to obtain raw logits
            output = model(input_tensor)
            # Convert logits to class probabilities using softmax
            # dim=1 corresponds to the class dimension (A / B)
            probs = torch.softmax(output, dim=1)
            # Extract:
            # 1. confidence – the highest predicted probability
            # 2. pred – the index of the predicted class (0 or 1)
            confidence, pred = torch.max(probs, dim=1)

            # Convert confidence from a PyTorch tensor to a Python float
            confidence = confidence.item()

            if confidence >= 0.9:
                # Convert the predicted class from tensor to Python int
                pred = pred.item()
