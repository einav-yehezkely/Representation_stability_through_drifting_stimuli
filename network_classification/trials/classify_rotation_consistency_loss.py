...
from shufflenet_v2_x0_5_consistency_loss import (
    train_model,
    get_dataloaders,
...
def plot_angle_concentration(
    ...
    """
    Plot concentration of predictions around base and opposite angles over all iterations.
    """
    ...
    plt.title(
        f"Concentration of correct predictions around base and opposite angles ({window_size}° slices)\n--{type_of_learning}--"
    )
...
def make_prev_dataloader(prev_root, batch_size=50):
    """
    prev_root structure:
      prev_root/train/A/*.jpg
      prev_root/train/B/*.jpg
    """
    if not os.path.exists(prev_root):
        return None

    train_path = os.path.join(prev_root, "train")
    if not os.path.exists(train_path):
        return None

    ds = datasets.ImageFolder(train_path, data_transforms["val"])
    if len(ds) == 0:
        return None
...
def save_small_prev_set(curr_split_root, prev_root, keep_per_class=120):
    """
    Take a small subset from curr_split_root/train/A and /train/B
    and copy it into prev_root/train/A and prev_root/train/B
    """
    # clear prev_root
    shutil.rmtree(prev_root, ignore_errors=True)
    os.makedirs(os.path.join(prev_root, "train", "A"), exist_ok=True)
    os.makedirs(os.path.join(prev_root, "train", "B"), exist_ok=True)

    for cls in ["A", "B"]:
        src_dir = os.path.join(curr_split_root, "train", cls)
        dst_dir = os.path.join(prev_root, "train", cls)
...
if __name__ == "__main__":
    ...
    for i in range(72):  # 360/5=72
        prev_root = inside_tmp("prev_small")  # קבוע לכל הריצה

        teacher_model = None
        prev_dataloader = None

        if i > 0:
            teacher_model = deepcopy(self_training_model).to(device)
            teacher_model.eval()

            prev_dataloader = make_prev_dataloader(prev_root, batch_size=50)
        ...
        a = pd.read_csv(inside_tmp("cluster_predicted_as_A.csv"))
        b = pd.read_csv(inside_tmp("cluster_predicted_as_B.csv"))
        print("cluster A/B sizes:", len(a), len(b), "cols:", a.columns.tolist())
        ...
        self_training_model = train_model(
            ...
            prev_dataloader=prev_dataloader,
            lambda_cons=0.05,
            T=2.0,
        )
        ...
        # save a small subset of the current training set to be used as "previous" data for the next iteration's consistency loss
        save_small_prev_set(
            curr_split_root=inside_tmp("split_data"),
            prev_root=prev_root,
            keep_per_class=120,
        )
