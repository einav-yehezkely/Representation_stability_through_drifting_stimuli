...
from resnet50_embeddings_training import (
    ResNet50Classifier,
    train_model,
...
# ============================================================
PCA_CSV = "pca_top2_filtered_female_vgg_1.csv"
...
PCA_DF["angle_deg"] = (
    np.degrees(
        np.arctan2(PCA_DF["y"], PCA_DF["x"])
    ) % 360
)

ANGLE_MAP = dict(
    zip(PCA_DF["filename"], PCA_DF["angle_deg"])
)
...
#
# Each embedding is 2048D.
# ============================================================

RESNET50_EMBEDDINGS_CSV = (
...
if _resnet50_df.shape[1] != expected_columns:

    raise RuntimeError(
        f"Expected {expected_columns} columns "
        f"(filename + {EMBEDDING_DIM} ResNet50 dimensions), "
        f"found {_resnet50_df.shape[1]}"
    )
...
for _, row in _resnet50_df.iterrows():
    ...
    # Same L2 normalization used during initial classifier training
    norm = np.linalg.norm(
        embedding
    )
...
print(
    "Loaded ResNet50 VGGFace2 embeddings:",
    len(_resnet50_df)
)
...
def get_resnet50_embedding(
    ...
    if filename in RESNET50_LOOKUP:

        return RESNET50_LOOKUP[
            filename
        ]


    if basename in RESNET50_LOOKUP:
    ...
    raise KeyError(
        f"ResNet50 VGGFace2 embedding "
        f"not found for: {filename}"
    )
...
def classify_images(model, csv_path, clusters=False):
    ...
    with torch.no_grad():
        for _, row in df.iterrows():
            ...
            try:
                embedding = get_resnet50_embedding(
                    filename
                )
            ...
            # 2D tensor of shape [1, 2048] for a single image embedding needed for the model input
...
def classify_filenames_batched(
    model,
    filenames,
    batch_size=300,
):
    ...
    with torch.no_grad():
        ...
        for filename in filenames:
            ...
            try:
                embedding = get_resnet50_embedding(filename)
            except KeyError:
                continue
...
def create_prediction_scatter(
    ...
    # Predictions on the full rotation sequence
    ...
    # Full rotation sequence predictions
    df_a = df[
        df["name"].isin(pred_a)
    ]
    ...
    # Images used for self-training, according to their pseudo-labels
    df_predicted_cluster_a = df[
        df["name"].isin(predicted_cluster_a)
    ]

    df_predicted_cluster_b = df[
        df["name"].isin(predicted_cluster_b)
    ]
...
if __name__ == "__main__":
    ...
    rotation_filenames = df_seq["filename"].tolist()
    ...
    for i in range(
        ...
        _, filenames_A = collect_nearest_images(
            base_point,
            points,
            names,
        ...
        _, filenames_B = collect_nearest_images(
            opposite_point,
            points,
            names,
        ...
        if UNSUPERVISED:
            ...
            merged_filenames = filenames_A + filenames_B

            rng = np.random.RandomState(42)
            rng.shuffle(merged_filenames)

            training_records = classify_filenames_batched(
                self_training_model,
                merged_filenames,
                batch_size=300,
            )
            ...
            labels = (
                [0] * len(filenames_pred_A)
                + [1] * len(filenames_pred_B)
            )
        ...
        if i % PLOT_EVERY == 0:
            ...
            print(
                f"rotation sequence classification time: "
                f"{time.time() - t:.2f}s"
            )
            ...
            if UNSUPERVISED:
                create_prediction_scatter(
                    angle=angle_deg,
                    frame_id=i,
                    predicted_cluster_a=filenames_pred_A,
                    predicted_cluster_b=filenames_pred_B,
                )
        ...
        if i % 100 == 0:
            print(
                f"Iteration {i} | LR = "
                f"{optimizer_ft.param_groups[0]['lr']}"
            )
