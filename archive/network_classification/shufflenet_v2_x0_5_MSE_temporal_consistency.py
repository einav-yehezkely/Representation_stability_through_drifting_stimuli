...
# Custom dataset class that extends ImageFolder to also return the filename of each image
...
def get_dataloaders(data_dir="split_data", batch_size=50):
    ...
    image_datasets = {
        x: ImageFolderWithFilenames(os.path.join(data_dir, x), data_transforms[x])
        for x in ["train", "val"]
    }
...
def evaluate(model, dataloader, criterion, device):
    ...
    for inputs, labels, filenames in dataloader:  # Iterate over batches
...
def train_model(
    ...
    with TemporaryDirectory() as tempdir:
        ...
        for epoch in tqdm(range(num_epochs), desc="Epoch Progress"):
            ...
            # Iterate over data.
            for inputs, labels, filenames in dataloaders["train"]:
                ...
                outputs = model(inputs).squeeze(1)
                probs = torch.sigmoid(outputs)  # prob_B
                preds = (probs >= 0.5).long()

                main_loss = criterion(probs, labels.float())
                loss = main_loss

                if prev_probs is not None:
                    prev_batch = torch.tensor(
                        [
                            prev_probs.get(fname, probs[j].detach().item())
                    ...
                    temporal_loss = torch.mean((probs - prev_batch) ** 2)
                    loss = main_loss + lambda_temporal * temporal_loss
