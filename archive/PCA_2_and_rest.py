####################################################################################################
# This script performs PCA (Principal Component Analysis) on 512D face embeddings extracted by FaceNet.
# It reduces dimensionality while preserving 95% of the variance, then identifies images that are
# most similar in the *residual space* (components 3 to k_95), i.e., not just in the dominant two PCs.
#
# Outputs:
# - `pca_95_percent_variance.csv`: Full projection onto top k_95 principal components.
# - `pca_top2.csv`: Projection onto PC1 and PC2 (used for visualization).
# - `pca_rest.csv`: Projection onto components 3 to k_95 (residual space).
# - `pca_rest_vectors_10_percent.csv`: The 10% of images closest to the origin in the residual space.
# - `pca_top2_filtered.csv`: The same images above, projected on PC1–PC2, for visualization.
# - `pca_eigenvalues.csv`: Eigenvalues and explained variance of all principal components.
#
# 28/05/2025
#################################################################
import pandas as pd
...
path = "female_facenet_vggface2_embeddings.csv"  # Path to the data file
# path = "female_resnet50_vggface2_embeddings.csv"
...
# print(
#     "Saved 10% of images with smallest squared radius in residual PCA space to pca_rest_vectors_10_percent.csv"
# )
